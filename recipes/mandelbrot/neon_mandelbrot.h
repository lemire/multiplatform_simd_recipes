...
static inline bool is_zero(uint32x4_t mask) {
    uint64x2_t v64 = vreinterpretq_u64_u32(mask);
    uint32x2_t v32 = vqmovn_u64(v64);
    uint64x1_t result = vreinterpret_u64_u32(v32);
    return vget_lane_u64(result, 0) == 0;
}
// See https://github.com/skeeto/mandel-simd/pull/3#issuecomment-377407425
int is_zero_bang(uint32x4_t mask) {
...
void
mandel_neon(uint8_t *image, const struct spec *s)
{
...
void
mandel_neon_bang(uint8_t *image, const struct spec *s)
{
    float32x4_t xmin = vdupq_n_f32(s->xlim[0]);
    float32x4_t ymin = vdupq_n_f32(s->ylim[0]);
    float32x4_t xscale = vdupq_n_f32((s->xlim[1] - s->xlim[0]) / s->width);
